---
title: "Project new - modeling departure"
author: "Tracy Chen"
date: "2023-11-22"
...
``` {r }
# dat <- read.csv("departure.csv", header = TRUE)
# 
...
# summary(dat)
# 
# # boxplot for cost class vs Departure delay
# p1 <- ggplot(dat, aes(x=DEP_DELAY_NEW)) +
#   geom_boxplot() +
#   labs(title = "Departure Delay by Cost Class",
#        x = "Departure Delay (minutes)") +
#   theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(), axis.title.y = element_blank()) +
#   facet_grid(CostClass ~ .) +
#   theme_minimal()
# 
# ggsave("depboxplot_eda1.png",p1, width = 6, height = 4)
# 
...
# dat <- dat %>%
#   filter(!(dat$DEP_DELAY_NEW > 6*60))
# 
# # save as rds file so that we don't need to reprocess the data every time
# saveRDS(dat, file="departure.Rda")
```
* Next we'll build our model to predict **Departure DELAY**
...
``` {r }
dat <- readRDS(file="departure.Rda")
...
dat.new <- dat %>%
  mutate_at(vars(-DEP_DELAY_NEW), ~ifelse(is.numeric(.), scale(.), .))
...
mod.ols <- lm(DEP_DELAY_NEW ~ ., data = training)
summary(mod.ols)
```
...
eval <- data.frame(model="OLS.removeNA", 
                   rmse.train= rmse(pred.ols,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.ols,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.ols,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.ols,testing$DEP_DELAY_NEW))
...
# plot cost class vs Departure delay again 
p2 <- ggplot(dat, aes(x=DEP_DELAY_NEW)) +
  geom_boxplot() +
  labs(title = "Departure Delay by Cost Class",
       x = "Departure Delay (minutes)") +
  facet_grid(CostClass ~ .) +
  theme_minimal()
ggsave("depboxplot_ols1.png",p2, width = 6, height = 4)
...
dat$DEP_DELAY_NEW <- log(dat$DEP_DELAY_NEW)
...
# replot the response
p3 <- ggplot(training, aes(x=DEP_DELAY_NEW)) +
  geom_boxplot() +
  labs(title = "Departure Delay by Cost Class",
       x = "Log-Transformed Departure Delay (minutes)") +
  facet_grid(CostClass ~ .) +
  theme_minimal()
ggsave("depboxplot_ols2.png",p3, width = 6, height = 4)
...
eval <- data.frame(model="AIC", 
                   rmse.train= rmse(pred.AIC,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.AIC,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.AIC,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.AIC,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)

eval <- data.frame(model="BIC", 
                   rmse.train= rmse(pred.BIC,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.BIC,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.BIC,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.BIC,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)
```
...
mod.pls <- plsr(DEP_DELAY_NEW ~., ncomp=10, data=training, validation="CV")
...
eval <- data.frame(model="PLS", 
                   rmse.train= rmse(pred.pls,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.pls,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.pls,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.pls,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)
```
...
non.dummy <- setdiff(non.dummy, "DEP_DELAY_NEW")
...
colnames(training.pcr)[ncol(training.pcr)] <- "DEP_DELAY_NEW"
mod.pcr <- pcr(DEP_DELAY_NEW ~ ., data=training.pcr, ncomp=2)

# apply testing data to pcr model
testing.pcr <- as.data.frame(cbind(testing.X,testing.Y))
colnames(testing.pcr)[ncol(testing.pcr)] <- "DEP_DELAY_NEW"
...
eval <- data.frame(model="PCR", 
                   rmse.train= rmse(pred.pcr,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.pcr,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.pcr,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.pcr,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)
...
training.famd$DEP_DELAY_NEW <- dat[training.idx,n]
testing.famd <- as.data.frame(famd.dat[testing.idx, , drop=FALSE])
testing.famd$DEP_DELAY_NEW <- dat[testing.idx,n]

# model
mod.famd <- lm(DEP_DELAY_NEW ~., data=training.famd)
sumary(mod.famd)
...
eval <- data.frame(model="FAMD", 
                   rmse.train= rmse(pred.famd,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.famd,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.famd,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.famd,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)
```
...
eval <- data.frame(model="Ridge", 
                   rmse.train= rmse(pred.ridge,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.ridge,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.ridge,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.ridge,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)
```
...
eval <- data.frame(model="Lasso", 
                   rmse.train= rmse(pred.lasso,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.lasso,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.lasso,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.lasso,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)
...
# 
# eval <- data.frame(model="group.lasso", rmse=rmse(pred.grouplasso,testing$DEP_DELAY_NEW), mae=mae(pred.grouplasso,testing$DEP_DELAY_NEW))
# mod.eval <- rbind(mod.eval, eval)
```
...
eval <- data.frame(model="SparseLasso", 
                   rmse.train= rmse(pred.sparselasso,training$DEP_DELAY_NEW), 
                   mae.train= mae(pred.sparselasso,training$DEP_DELAY_NEW),
                   rmse.test= rmse(pred.sparselasso,testing$DEP_DELAY_NEW), 
                   mae.test= mae(pred.sparselasso,testing$DEP_DELAY_NEW))
mod.eval <- rbind(mod.eval, eval)
```
